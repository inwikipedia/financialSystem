(function () {
    ...
    function LineEditor(container, data, name, resources, borderChangedCallback) {
        ...
        this.updateUI(data.borderWidth);
        ...
        this.beginArrowSizeEditor = new ImgDropEditor(container, data.beginArrowheadSize, {
            normal: '../images/shapeArrowSizeIcon.png',
    ...
    LineEditor.prototype.updateUI = function (width) {
    ...
    ImgDropEditor.prototype._createDefaultElement = function (defaultImgUrlObj) {
        ...
        //     if ($(e.target).closest(self.itemsImgList).length === 0 && $(e.target).closest(button).length === 0) {
        //         button.css({
        //             "background-image": "url(" + defaultImgUrlObj.normal + ")",
        //         });
        //     }
...
//#region PropertyTabHeader
    function PropertyTabHeader(container, headers, tabChangedCallback) {
        ...
        headers.forEach(function (header) {
            categoryGroup.append(self.addTabHeader(header.name));
        });
        this.activePropertyTabHeader = $(categoryGroup.children()[0]);
        this.activePropertyTabHeader.children().remove();
        this.activePropertyTabHeader.append($('<span></span>').addClass('selected-' + _buildPanelContentCss(this.activePropertyTabHeader.attr('title'))).addClass('format-shape-panel'));
    ...
    PropertyTabHeader.prototype.addTabHeader = function (name/*, icon*/) {
        var self = this;
        var categorySpan = $('<span></span>').addClass(_buildPanelContentCss(name)).addClass('format-shape-panel');
        var div = $('<div></div>').append(categorySpan).attr('title', name).addClass('format-shape-panel-background');
        div.click(function (event) {
...
//#region PropertyTabPanel
    ...
    PropertyTabPanel.prototype.hidePropertyGroupPanel = function (oldPropertyGroupPanel) {
        ...
        this.propertyHeaderPanel.activePropertyTabHeader.append($('<span></span>').addClass(_buildPanelContentCss(this.propertyHeaderPanel.activePropertyTabHeader.attr('title'))).addClass('format-shape-panel'));
    };
    PropertyTabPanel.prototype.showPropertyGroupPanel = function (newPropertyGroupPanel, name) {
        ...
        for (var i = 0; i < headerElements.length; i++) {
            if ($(headerElements[i]).attr("title") === name) {
                ...
                this.propertyHeaderPanel.activePropertyTabHeader.children().remove();
                this.propertyHeaderPanel.activePropertyTabHeader.append($('<span></span>').addClass('selected-' + _buildPanelContentCss(this.propertyHeaderPanel.activePropertyTabHeader.attr('title'))).addClass('format-shape-panel'));
            }
        }
    };
