(function () {
    ...
    if (permissionTitle.indexOf('新增项目') != -1) {
        $('#adcformdesign-table-td').find('select').attr('disabled', true);
        layui.form.render();
    }
    ...
    dateInput.each(function () {
        if ($(this)[0].getAttribute('adcform_listctrlmark') == '') {
            inputDom.push($(this));
        }else if($(this)[0].getAttribute('adcform_listctrlmark') != ''){
    ...
    var account = layui.config.getAccount().usid;
    if(permissionTitle.indexOf('新增项目') == -1 && permissionTitle.indexOf('启动流程') == -1){
    //获取启动流程时存入的年月数据，渲染上下两个表单的年月
    $.ajax({
        url: "/api/project/projectSchedule/selectByUserDeptAndProType?processInstId=" + processInstanceId + "&projectType=1-执行情况&classType=1",
        type: 'POST',
    ...
    //第一个列表控件的数据回显
    $.ajax({
        url: "/api/project/projectSchedule/selectByUserDeptAndProType?processInstId=" + processInstanceId + "&projectType=行业工作&classType=1",
        type: 'POST',
        contentType: 'application/json',
        dataType: 'json',
        async: false,
    ...
    //第二个列表控件的数据回显
    $.ajax({
        url: "/api/project/projectPlan/selectByUserDeptAndProType?processInstId=" + processInstanceId + "&projectType=行业工作&classType=1",
        type: 'POST',
        contentType: 'application/json',
        dataType: 'json',
        async: false,
        success: function (data) {
            ...
            tableRender(tmpDom[1], list, inputDom[2], inputDom[3]);
        },
        error: function () {
            layui.msg('获取数据失败');
            return;
    ...
    //时间控件自动保存方法
    var tableDateControl = tmpDom[1].find('[name = "datetime"]');
        ...
        layui.laydate.render({
            elem: "#" + id,
                ...
                for (var dd = 0; dd < tr1.find('td').length; dd++) {
                    ...
                    quarterFormHelper.inputStr(inputArray, inputType, domI, str);
                    inputArray = [];
                }
                ...
                str.extInf02 = '1';
                var message = quarterFormHelper.notNull($(that));
                if(message){
                    quarterFormHelper.transferData(str,tr1[0]);
                }else{
                    return;
                }
            }
    ...
    if (tmpDom[1][0].children[0].children[1].style.display == 'none') {
        ...
        $('#adcformdesign-table-td').find('[adcform_listctrlmark="列表控件1"]')[0].value = tmpDom[1].children().children().length - 1;
    }
    ...
    //     quarterFormHelper.dateJudge(inputDom[2], inputDom[3], tmpDom[1]);
    // })
    ...
    //     quarterFormHelper.dateJudge(inputDom[0], inputDom[1], tmpDom[0]);
    // })
    ...
    remark1.each(function () {
        $(this).on('blur', function () {
            ...
            for (var i = 0; i < $(tr1).find('td').length; i++) {
                ...
                quarterFormHelper.inputString(inputArray, inputType, domI, str);
                inputArray = [];
            }
            var tableid = table[0].getAttribute('tableid');
            ...
            str.extInfo2 = '1';
            var message = quarterFormHelper.notNull($(tr1.children[0].children[0]));
            if(message){
                quarterFormHelper.transData(str,tr1);
            }else{
                return;
            }
        })
    ...
    table1.each(function () {
        ...
        if ($(this).parent().parent()[0].getAttribute('name') != 'remark') {
            $(this).on('blur', function () {
                ...
                str.extInfo1 = processInstanceId;
                str.extInfo2 = '1';
                var message = quarterFormHelper.notNull($(this));
                if(message){
                    quarterFormHelper.transData(str,tr1[0]);
                }else{
                    return;
                }
            })
    ...
    deleteEm1.each(function(){
        $(this).on('click',function () { 
            ...
            if(tr1[0].getAttribute('markId') == undefined){
                ...
                quarterFormHelper.deleteData(id);
            }
         })
    })   
    var table3 = tmpDom[1].find('input');
    //为table3每一个input加一个方法
    table3.each(function () {
        //失去焦点自动保存
        if ($(this).parent()[0].getAttribute('name') != 'datetime' && $(this).parent()[0].localName != 'div') {
            $(this).on('blur', function () {
                var str = {
                    "cooperationDepartment": "",
                    "department": "",
    ...
    var table4 = tmpDom[1].find('textarea');
    ...
    var table5 = tmpDom[1].find('select');
    table5.each(function () {
        var filter = $(this)[0].getAttribute('name');
        $(this).attr('lay-filter', filter);
        layui.form.on('select(' + filter + ')', function () {
            var str = {
                "cooperationDepartment": "",
                "department": "",
                "extInf01": "",
            ...
            var inputArray = new Array();
            var tr1 = $(this).parent().parent().parent().parent();
            for (var i = 0; i < tr1.find('td').length; i++) {
                var inputName = tr1[0].children[i].children[0].getAttribute('name');
                inputArray.push(inputName);
            ...
            str.extInf01 = processInstanceId;
            str.extInf02 = '1';
            var message = quarterFormHelper.notNull($(this).parent().parent());
            if(message){
                quarterFormHelper.transferData(str,tr1[0]);
            }else{
                return;
    ...
    deleteEm2.each(function(){
        $(this).on('click',function () { 
            ...
            if(tr1[0].getAttribute('markId') == undefined){
                ...
                var id = tr1[0].getAttribute('markId');
                quarterFormHelper.deleteDataPlan(id);
            }
         })
    })  
                ...
                if ($(this).parent().parent()[0].getAttribute('name') != 'remark') {
                    $(this).on('blur', function () {
                        ...
                        for (var i = 0; i < tr1.find('td').length; i++) {
                            ...
                            var domI = tr1.find('textarea')[i];
                            quarterFormHelper.inputString(newinputArray, newinputType, domI, str);
                            newinputArray = [];
                        }
                        str.year = inputDom[0][0].getAttribute('name') + "^^" + "year" + "^^" + inputDom[0][0].value;
                        str.month = inputDom[1][0].getAttribute('name') + "^^" + "month" + "^^" + inputDom[1][0].value;
    ...
    addicon2.each(function () {
        $(this).on('click', function () {
            ...
            var newselect = $(newDom).find('select');
            ...
            newinput.each(function () {
                // console.log($(this));
                if ($(this)[0].getAttribute('class') != 'layui-input layui-unselect'){
                    $(this).on('blur', function () {
                        var str = {
                            "cooperationDepartment": "",
                            "department": "",
                        ...
                        for (var i = 0; i < tr1.find('td').length; i++) {
                            ...
                            var newinputType = table[0].children[0].children[0].cells[i].innerText;
                            var domI = tr1[0].children[i].children[0];
                            quarterFormHelper.inputStr(newinputArray, newinputType, domI, str);
                            newinputArray = [];
                        }
                        str.year = inputDom[2][0].getAttribute('name') + "^^" + "year" + "^^" + inputDom[2][0].value;
                        str.month = inputDom[3][0].getAttribute('name') + "^^" + "month" + "^^" + inputDom[3][0].value;
...
var quarterFormHelper = {
    ...
    inputStr: function (inputArray, inputType, obj, str) {
        ...
        } else if (inputType.indexOf('类别') != -1) {
    ...
    dateJudge: function (dom1, dom2, dom3) {
...
function tableRender(tmpDom, list, dom1, dom2) {
    ...
    for (var i = 0; i < list.length; i++) {
        ...
        for (var name in tableData) {
            ...
            if (tableData[name] == 'input') {
                ...
                newcode += "<td name='select'>" + "<select name='" + myStom[1] + "' value = '" + myStom[0] + "' id = '" + "select_" + name + "_" + (j + 2) + "'>" + oparr + "</select>" + "</td>";
                selectNum += "select_" + name + "_" + (j + 2) + '`';
                selectData += myStom[0] + '`';
        ...
        if (tr3.getAttribute('name') == 'remark') {
            // $(tmpDom[0].children[0].rows[j - 2]).after(newcode);
    ...
    if (selectNum != '') {
        ...
        selectNum = selectNum.substr(0, selectNum.length - 1);
        selectNum = selectNum.split("`");
        for (var p = 0; p < selectNum.length; p++) {
            $("#" + selectNum[p]).find("option[value='" + selectData[p] + "']").prop('selected', true);
        }
    }
    layui.form.render();
}