        ...
        function($rootScope, $timeout, $modal, $translate, $location, $window, $http, $q) {
            ...
            function fetchModel(modelId) {
                ...
                $http({method: 'GET', url: modelUrl}).
                    success(function (data, status, headers, config) {
                        console.log("!@#",data, status, headers, config,$rootScope);
            ...
            $rootScope.$on('$includeContentLoaded', function (event) {
	            if (!$rootScope.editorInitialized) {
	                ...
	                var modelId = EDITOR.UTIL.getParameterByName('modelId'); //对modelID进行处理
	                fetchModel(modelId);  //根据id获取对应的模型json数据
	                ...
	                jQuery(window).resize(function () {
	                    ...
	                    var totalAvailable = jQuery(window).height() - offset.top - mainHeader.height() - 24;

	                    //canvas.height(totalAvailable - propSectionHeight);
                        //alert(jQuery(window).height());
                        canvas.height(jQuery(window).height()-'60');
                        jQuery(".selected-item-body").height(jQuery(window).height()-'90');
	                    ...
	                    if(actualCanvas) {
	                        ...
	                        //todo im not sure it will whether solve the icon position problem or not
                            if (!actualCanvas.clientWidth||actualCanvas.clientWidth<actualCanvas.parentNode.clientWidth) {
	                            actualWidth = actualCanvas.parentNode.clientWidth;
                            } else {
                                actualWidth = actualCanvas.clientWidth;
                            }

                            //actualWidth = actualCanvas.clientWidth || actualCanvas.parentNode.clientWidth;
	                        //console.log(actualCanvas.clientWidth,actualCanvas.parentNode.clientWidth);
	                    }
	                    ...
	                    east.css('top', canvasTop + 10 + (canvasHeight - iconWidth) / 2 +  'px');
	                    //console.log("3",canvasLeft , canvasWidth , iconOffset , iconCenterOffset);
	                    east.css('left', (canvasLeft + canvasWidth - iconOffset - iconCenterOffset) + 'px');
