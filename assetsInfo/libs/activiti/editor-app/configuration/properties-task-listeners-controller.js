...
var KisBpmTaskListenersPopupCtrl = [ '$scope', '$http', '$q', '$translate', function($scope, $http, $q, $translate) {
    ...
    $http({method: 'GET', url: KISBPM.URL.getListener()}).success(function (data, status, headers, config) {
        if(data.ok == true){
            console.log(":::",data);
            $scope.listenerItems = data.data;

        }
    }).
    error(function (data, status, headers, config) {
        console.log('KisBpmTaskListenersPopupCtrl is wrong!');
    });

    $scope.listenerExpressionChanged = function() {
