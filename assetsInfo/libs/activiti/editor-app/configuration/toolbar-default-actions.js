...
var SaveModelCtrl = ['$rootScope', '$scope', '$http', '$route', '$location',
    function ($rootScope, $scope, $http, $route, $location) {
        ...
        $scope.save = function (successCallback) {
            ...
            for (var x = 0; x < json.childShapes.length; x++) {
                if (json.childShapes[x].stencil.id == "Pool") {
                    json.childShapes[x].properties.name = json.properties.name;
                }
                if (json.childShapes[x].stencil.id == "UserTask") {
                    if (json.childShapes[x].properties.usertaskgroup
                        && json.childShapes[x].properties.usertaskassignment.assignment.candidateGroups == null) {
                        var roleId = json.childShapes[x].properties.usertaskgroup;
                        json.childShapes[x].properties.usertaskassignment.assignment = {};
                        //$$hashKey作用不明
                        var assignment = '{"assignment": {"candidateGroups": [{"value":"' + roleId + '","$$hashKey":"' + roleId + '"}]}}';
                        json.childShapes[x].properties.usertaskassignment = JSON.parse(assignment);
                    }

                    if (!json.childShapes[x].properties.usertaskassignment && json.childShapes[x].properties.multiinstance_type == "Parallel" || json.childShapes[x].properties.multiinstance_type == "Sequential") {
                        json.childShapes[x].properties.usertaskassignment = {};
                        json.childShapes[x].properties.usertaskassignment.assignment = {};
                        json.childShapes[x].properties.usertaskassignment.assignment.assignee = "${assignee}";
                    }
                    //修正上次错误，在设置为流时将assignee设置为initiator
                    if (json.childShapes[x].properties.setvalueinitiator != "false" && json.childShapes[x].properties.setvalueinitiator != false) {
                        json.childShapes[x].properties.usertaskassignment = {};

                        json.childShapes[x].properties.usertaskassignment.assignment = {};

                        json.childShapes[x].properties.usertaskassignment.assignment.assignee = "${initiator}";
                    }
                }
            }
            console.log('!', json);
            ...
            $http({
                ...
                .success(function (data, status, headers, config) {
                    ...
                    //$scope.updatePropertyInModel($scope.modelData.model.properties);
                    $scope.editor.handleEvents({
                        type: ORYX.CONFIG.EVENT_SAVED
                    });
                    $scope.modelData.name = $scope.saveDialog.name;
                    $scope.modelData.model.properties.name = $scope.saveDialog.name;
                    ...
                    console.log($rootScope.modelData);
                    //$scope.updatePropertyInModel($rootScope.modelData.properties);

                    // var modelUrl = KISBPM.URL.getModel($scope.modelData.modelId);
                    //
                    // $http({method: 'GET', url: modelUrl}).
                    // success(function (data, status, headers, config) {
                    //     console.log(data,$rootScope);
                    //     $rootScope.editor = new ORYX.Editor(data);
                    //     $rootScope.tmodelDaa = angular.fromJson(data);
