(function() {
    ...
    callWithJQuery(function($) {
        ...
        aggregators = (function(tpl) {
            return {
                "数量": tpl.count(usFmtInt),
                // "计算唯一值": tpl.countUnique(usFmtInt),
                // "列出唯一值": tpl.listUnique(", "),
                // "和": tpl.sum(usFmt),
                // "整数和": tpl.sum(usFmtInt),
                // "平均": tpl.average(usFmt),
                // "中位数": tpl.median(usFmt),
                // "样本差异": tpl["var"](1, usFmt),
                // "样本标准差": tpl.stdev(1, usFmt),
                // "最小值": tpl.min(usFmt),
                // "最大值": tpl.max(usFmt),
                // "正序": tpl.first(usFmt),
                // "倒序": tpl.last(usFmt),
                // "总和": tpl.sumOverSum(usFmt),
                // "80%上限": tpl.sumOverSumBound80(true, usFmt),
                // "80%下限": tpl.sumOverSumBound80(false, usFmt),
                // "总和作为总数的分数": tpl.fractionOf(tpl.sum(), "total", usFmtPct),
                // "总和作为行的分数": tpl.fractionOf(tpl.sum(), "row", usFmtPct),
                // "总和作为列的分数": tpl.fractionOf(tpl.sum(), "col", usFmtPct),
                // "数量作为总数的分数": tpl.fractionOf(tpl.count(), "total", usFmtPct),
                // "数量作为行的分数": tpl.fractionOf(tpl.count(), "row", usFmtPct),
                // "数量作为列的分数": tpl.fractionOf(tpl.count(), "col", usFmtPct)
            };
        })(aggregatorTemplates);
        renderers = {
            ...
            // "条形图": function(data, opts) {
            //     return $(pivotTableRenderer(data, opts)).barchart();
            // },
            // "热图": function(data, opts) {
            //     return $(pivotTableRenderer(data, opts)).heatmap("heatmap", opts);
            // },
            // "行热图": function(data, opts) {
            //     return $(pivotTableRenderer(data, opts)).heatmap("rowheatmap", opts);
            // },
            // "热映射图": function(data, opts) {
        ...
        locales = {
            en: {
                ...
                renderers: renderers,
                localeStrings: {
                    renderError: "呈现数据透视表结果时出错。",
                    computeError: "计算数据透视表结果时出错。",
                    uiRenderError: "呈现数据透视表用户界面时出错。",
                    selectAll: "全选",
                    selectNone: "取消全选",
                    tooMany: "(数据过多)",
                    filterResults: "筛选值",
        ...
        pivotTableRenderer = function(pivotData, opts) {
            ...
            colAttrs = pivotData.colAttrs; //行元素
            console.log(pivotData)
            rowAttrs = pivotData.rowAttrs //""; //列元素
            rowKeys = pivotData.getRowKeys(); //列值
            colKeys = pivotData.getColKeys(); //行值
