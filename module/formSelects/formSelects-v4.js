...
(function (layui, window, factory) {
	...
	Common.prototype.getChildrenList = function (arr, childrenName, disabledName, pid, disabled) {
		var result = [],
		    offset = 0;
		for (var a = 0;arr && a < arr.length; a++) {
			var item = arr[a];
	...
	Common.prototype.commonHandler = function (key, label) {
		...
		//title值
		label.parents('.' + FORM_TITLE + ' .' + NAME).attr('title', ADCFormDesignHelper.htmlDecodeByRegExp(data[key].values.map(function (val) {
			return val.name;
		}).join(',')));
	};
	...
	Common.prototype.addLabel = function (id, div, val) {
		...
		if (val.name.length>=6){
			subStr = val.name.substring(0,6)+'...';
		}else {
			subStr = val.name;
		}
		var _ref = [$('<span ' + tips + ' value="' + val.value + '"><font ' + tips + '>' + subStr + '</font></span>'), $('<i ' + tips + ' class="xm-iconfont icon-close"></i>')],
		    $label = _ref[0],
		    $close = _ref[1];
	...
	Common.prototype.selectAll = function (id, isOn, skipDis) {
		...
		dl.find('dd[lay-value]:not(.' + FORM_SELECT_TIPS + '):not(.' + THIS + '):not(.'+DD_HIDE+')' + (skipDis ? ':not(.' + DISABLED + ')' : '')).each(function (index, item) {
			item = $(item);
